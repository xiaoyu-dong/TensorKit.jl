...

{
    ...
    "category": "section",
    "text": "Pages = [\"man/intro.md\", \"man/spaces.md\", \"man/sectors.md\"]\nDepth = 3"
},
...

{
    ...
    "title": "Bibliography",
    "category": "section",
    "text": "[tung]: Tung, W. K. (1985). Group theory in physics: an introduction to symmetry principles,\n    group representations, and special functions in classical and quantum physics.\n    World Scientific Publishing Company.[selinger]: Selinger, P. (2010). A survey of graphical languages for monoidal categories.\n        In New structures for physics (pp. 289-355). Springer, Berlin, Heidelberg."
},
...

{
    ...
    "category": "section",
    "text": "Let the different irreps or sectors be labeled as a, b, c, … First and foremost, we need to specify the fusion rules a  b =  N_ab^c c with N_{a,b}^c some non-negative integers. There should always exists a unique trivial sector u such that a  u = a = u  a. Furthermore, there should exist a unique sector overlinea such that N_aoverlinea^u = 1, whereas for all b  overlinea, N_ab^u = 0. For example, for the representations of mathsfSU_2, all irreps are self-dual (i.e. a = overlinea) and the trivial sector corresponds to spin zero.The meaning of the fusion rules is that the space of transformations R_a  R_b  R_c (or vice versa) has dimension N_ab^c. In particular, we assume the existence of a basis consisting of unitary tensor maps X_ab^cμ  R_c  R_a  R_b with μ = 1, \\ldots, N_{a,b}^c such that(X_ab^cμ)^ X_ab^cμ = mathrmid_R_candsum_c sum_μ = 1^N_ab^c X_ab^cμ (X_ab^cμ)^dagger = mathrmid_R_a  R_bThe tensors X_ab^cμ are the splitting tensors, their hermitian conjugate are the fusion tensors. They are only determined up to a unitary basis transform within the space, i.e. acting on the multiplicity label μ. For mathsfSU_2, where N_ab^c is zero or one and the multiplicity labels are absent, the entries of X_ab^c are precisely given by the CG coefficients. The point is that we do not need to know the tensors X_ab^cμ, the topological data of (the representation category of) the group describes the following transformation:F-move or recoupling: the transformation between (a  b)  c to a  (b  c):\n(X_ab^eμ  mathrmid_c)  X_ec^dν = _fκλ F^abc_d^eμν_fκλ (X_ab^eμ  mathrmid_c) X_ec^dν (mathrmid_a  X_bc^fκ)  X_af^dλ\nBraiding or permuting as defined by σ_ab R_a  R_b  R_b  R_a:\n``σ{Ra,Rb} ∘ X{a,b}^{c,μ} = ∑{ν} [R{a,b}^c]^μν X{b,a}^{c,ν}The dimensions of the spaces R_a on which representation a acts are denoted as d_a and referred to as quantum dimensions. In particular d_u = 1 and d_a = d_overlinea. This information is also encoded in the F-symbol as d_a =  F^a overlinea a_a^u_u ^-1. Note that there are no multiplicity labels in that particular F-symbol as N_{a,\\overline{a}}^u = 1.If, for every a and b, there is a unique c such that a  b = c (i.e. N_ab^c = 1 and N_ab^c = 0 for all other c), the category is abelian. Indeed, the representations of a group have this property if and only if the group multiplication law is commutative. In that case, all spaces R_a associated with the representation are one-dimensional and thus trivial. In all other cases, the category is nonabelian. We find it useful to further finegrain between categories which have all N_ab^c equal to zero or one (such that no multiplicity labels are needed), e.g. the representations of mathsfSU_2, and those where some N_ab^c are larger than one, e.g. the representations of mathsfSU_3.Using R_u  ℂ, λ_R_η{R{\\overline{a}}}``If, for every a and b, there is a unique c such that a  b = c (i.e. N_ab^c = 1 and N_ab^c = 0 for all other c), the category is Abelian. Indeed, the representations of a group have this property if and only if the group multiplication law is commutative. In that case, all spaces R_a associated with the representation are one-dimensional and thus trivial.Furthermore, there is a relation between splitting vertices and fusion vertices given by the B-symbol, but we refer to the section on Fusion trees for the precise definition and further information. The required data is completely encoded in the the F-symbol, and corresponding Julia function Bsymbol(a,b,c)There should always exists a unique trivial sector u such that a  u = a = u  a. Furthermore, there should exist a unique sector overlinea such that N_aoverlinea^u = 1, whereas for all b  overlinea, N_ab^u = 0. For example, for the representations of mathsfSU_2, all irreps are self-dual (i.e. a = overlinea) and the trivial sector corresponds to spin zero.The dimensions of the spaces R_a on which representation a acts are denoted as d_a and referred to as quantum dimensions. In particular d_u = 1 and d_a = d_overlinea."
},
